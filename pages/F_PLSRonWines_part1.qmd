...
The scree plot shows the eigenvalues of each dimension. These eigenvalues give the squared *coefficient of regression* ($\beta^2$ = `r NA`) of each pair of latent variables. In other words, the singular values, which are the square root of the eigenvalues, give the *coefficient of regression* ($\beta$ = `r NA`) of these pairs of latent variables. The sum of the eigenvalues is equal to the sum of the squared correlation between all variables in both tables.
